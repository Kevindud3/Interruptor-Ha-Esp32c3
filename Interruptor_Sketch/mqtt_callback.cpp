// Mensagens do MQTT são recebidas aqui, nossas mensagens são de texto simples a
// função callback aqui lé a mensagem e a reescreve na variável message que é
// uma string. E a imprime para debug.
...
void setupPWM() { ledcAttach(pinoPWM, pwmFreq, pwmResolution); }
...
void callback(char *topic, byte *payload, unsigned int length) {
  ...
  if (strcmp(topic, "lampada/brilho") == 0) {
    int brilho =
        constrain(message.toInt(), 0, 100); // Garantir que está entre 0-100
